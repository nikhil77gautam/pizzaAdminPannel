...
const Home = () => {
  ...
  const allAdminPizza = useSelector(
    (state) => state.getAdminPizzas.readAllpizza
  );
  ...
  const handleDeletePizza = async () => {
    if (selectedPizza) {
      try {
        await axios.post(
          `https://pizzabackend-0x3r.onrender.com/deletepizzas/${selectedPizza._id}`
        );
  ...
  return (
    <div className="p-6">
      <div>
        ...
        {updatePizzaPopupId && (
          <Updatepizza
            pizzaId={updatePizzaPopupId}
            updatePizzaPopupClose={updatePizzaPopupClose}
          />
        )}
